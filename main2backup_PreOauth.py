    ...
    if text == "ลงทะเบียน":
        ...
        if employee_data: # User already exists
            ...
            line_bot_api.reply_message(
                ReplyMessageRequest(
                    reply_token=event.reply_token,
                    messages=[V3TextMessage(text="กรุณาพิมพ์ชื่อ-นามสกุลและตำแหน่งงานของคุณ คั่นด้วยคอมม่า (,) เช่น สมชาย ใจดี, ผู้จัดการ")]
                )
            )
        ...
        if not employee_data: # User not registered, process as registration input
            ...
            if len(parts) == 2:
                ...
                line_bot_api.reply_message(
                    ReplyMessageRequest(
                        reply_token=event.reply_token,
                        messages=[V3TextMessage(text="รูปแบบข้อมูลไม่ถูกต้อง กรุณาพิมพ์ชื่อ-นามสกุลและตำแหน่งงานของคุณ คั่นด้วยคอมม่า (,) เช่น สมชาย ใจดี, ผู้จัดการ")]
                    )
                )
